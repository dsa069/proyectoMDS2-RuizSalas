package interfaz;

public class Seleccion_de_secciones_Vista_Editor_item extends Seleccion_de_secciones_item {
	public Seleccion_de_secciones_Vista_Editor _seleccion_de_secciones__Vista_Editor_;
	
	public Seleccion_de_secciones_Vista_Editor_item(Seleccion_de_secciones_Vista_Editor padre, ocl_proyecto.Seccion seccion) {
		super(padre, seccion);
		this._seleccion_de_secciones__Vista_Editor_ = padre;
	}
}